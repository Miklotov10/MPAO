<div class="spaces index">
	<h2><?php echo __('Spaces'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<thead>
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('name'); ?></th>
			<th><?php echo $this->Paginator->sort('facility_id'); ?></th>
			<th><?php echo $this->Paginator->sort('gender_id'); ?></th>
			<th><?php echo $this->Paginator->sort('male_count'); ?></th>
			<th><?php echo $this->Paginator->sort('female_count'); ?></th>
			<th><?php echo $this->Paginator->sort('total_count'); ?></th>
			<th><?php echo $this->Paginator->sort('other_facility'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php foreach ($spaces as $space): ?>
	<tr>
		<td><?php echo h($space['Space']['id']); ?>&nbsp;</td>
		<td><?php echo h($space['Space']['name']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($space['Facility']['name'], array('controller' => 'facilities', 'action' => 'view', $space['Facility']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($space['Gender']['name'], array('controller' => 'genders', 'action' => 'view', $space['Gender']['id'])); ?>
		</td>
		<td><?php echo h($space['Space']['male_count']); ?>&nbsp;</td>
		<td><?php echo h($space['Space']['female_count']); ?>&nbsp;</td>
		<td><?php echo h($space['Space']['total_count']); ?>&nbsp;</td>
		<td><?php echo h($space['Space']['other_facility']); ?>&nbsp;</td>
		<td><?php echo h($space['Space']['created']); ?>&nbsp;</td>
		<td><?php echo h($space['Space']['modified']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $space['Space']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $space['Space']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $space['Space']['id']), array(), __('Are you sure you want to delete # %s?', $space['Space']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Space'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Facilities'), array('controller' => 'facilities', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Facility'), array('controller' => 'facilities', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Genders'), array('controller' => 'genders', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Gender'), array('controller' => 'genders', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Rental Promos'), array('controller' => 'rental_promos', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Rental Promo'), array('controller' => 'rental_promos', 'action' => 'add')); ?> </li>
	</ul>
</div>
